	...
	switch (status.type) {
		...
		case SiteStatusTag.DISABLED_TEMPORARILY:
			return (
				<Chip
					size="small"
					label={`Off for ${readableDuration(status.until - Date.now())}`}
					variant="outlined"
				/>
			);
	}
...
function DisableConfirm({ siteId }: { siteId: SiteId }) {
	...
	return (
		...
		>
			...
			<Button size="small" variant="outlined" onClick={onConfirm(5 * MINUTE)}>
				5 min
			</Button>
			<Button size="small" variant="outlined" onClick={onConfirm(HOUR)}>
				1 hr
			</Button>
			<Button size="small" variant="outlined" onClick={onConfirm('forever')}>
				Forever
			</Button>
		</Stack>
	);
	...
	const onEnableAll = React.useCallback(async () => {
		try {
			const allOrigins = Array.from(
				new Set(Object.values(Sites).flatMap((s) => s.origins))
			);
			const granted = await getBrowser().permissions.request({
				permissions: [],
				origins: allOrigins,
			});
			if (granted) {
			...
			<Container maxWidth="sm" sx={{ py: 4 }}>
				<Box sx={{ mb: 3, display: 'flex', justifyContent: 'center' }}>
					...
					>
						...
						<Box
							component="img"
							src="transparent-icon-green.png"
							alt=""
							sx={{ height: '0.9em', width: 'auto', opacity: 0.9 }}
						/>
					</Typography>
				</Box>
				<Box
				...
				>
					<Box sx={{ p: 2 }}>
						...
						{showEnableAll && (
							<Box sx={{ mt: 1.5 }}>
								<Button size="small" variant="contained" onClick={onEnableAll}>
									Enable All
								</Button>
							</Box>
						)}
