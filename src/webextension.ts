	...
	if (typeof browser !== 'undefined') {
		...
		return {
			...
			tabs: {
				...
				query: (q) =>
					new Promise((resolve) =>
						chrome!.tabs?.query(q as any, resolve as any)
					),
				remove: (tabId) =>
					new Promise((resolve) => chrome!.tabs?.remove(tabId as any, resolve)),
				update: (tabId, props) =>
					new Promise((resolve) =>
						chrome!.tabs?.update(tabId as any, props as any, resolve as any)
					),
			},
